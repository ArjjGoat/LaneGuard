...
def average_slope_intercept(image, lines):
    ...
    return np.array(
        [make_coordinates(image, left_avg), make_coordinates(image, right_avg)]
    )
...
def make_coordinates(image, line_parameters):
    ...
    y1 = image.shape[0]
    y2 = int(y1 * (3.5 / 5))  # Adjusted to make lines longer
...
def lane_detection(frame):
    ...
    lines = cv2.HoughLinesP(
        roi, 2, np.pi / 180, 100, np.array([]), minLineLength=100, maxLineGap=100
    )
...
def main():
    ...
    while True:
        ...
        now = time.time_ns()
        if (
            abs(averaged_lines[1][0] - width >= 425)
            or abs(averaged_lines[0][0] - width >= 425)
        ) and (now - init_time) > 300000000:
            init_time = now
            counter += 1
            print(counter)
            mixer.init()
