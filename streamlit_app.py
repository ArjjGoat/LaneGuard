...
def lane_detection_page():
    ...
    input_type = st.radio(
        "Select input type:", ("Live Video", "Upload MP4"), horizontal=True
    )
    ...
    if st.session_state.show_disclaimer:
        disclaimer_placeholder.markdown(
            '<div class="disclaimer">Road isn\'t detected, please direct camera towards the road!</div>',
...
def laneguard_demo_page():
    ...
    st.write(
        "This demo shows the output of our lane detection algorithm on a pre-processed video."
    )
    ...
    with st.expander("About Laneguard"):
        ...
        """)
        st.success(
            "Laneguard has been tested on various road conditions and weather scenarios."
        )
